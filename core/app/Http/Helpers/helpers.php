...
function advertisements($size) {
    ...
    if ($ad) {
        ...
        if ($ad->type == 1) {
            return  '<a  target="_blank" href="'.route('add.clicked',encrypt($ad->id)).'" class="mt-3 d-block"><img  src="'.getImage('assets/images/advertisement/'.$ad->image).'" alt="image" class="w-100"></a>';            
        }
        if($ad->type == 2) {
