                    ...
                    <div class="main-content">
                            ...
                            <div class="colxl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                <div class="page-title-wrapper">
                                    <div class="page-title-box">
                                        <h4 class="page-title bold"><?php echo $this->lang->line('ltr_land_marks');?></h4>
                                    </div>
                                    <div class="breadcrumb-list">
                                        <ul>
                                            ...
                                            <li class="breadcrumb-link active"><?php echo $this->lang->line('ltr_add_landmarks');?></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                <div class="card table-card">
                                    <div class="card-header pb-0">
                                        <h4><?php echo $this->lang->line('ltr_add_landmarks');?></h4>
										<div class="error"></div>
                                    </div>
                                    <div class="card-body pb-4">
                                        <div class="chart-holder">
                                            <form class="separate-form formget" method="POST" action="<?=base_url('admin/addlandmark');?>">
                                                <input type="hidden" name="update_id" value="<?=(isset($update_landmarks[0]['id']) && !empty($update_landmarks[0]['id']))?$update_landmarks[0]['id']:'';?>">
                                                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                                    <div class="row">
                                                        <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12">
                                                            <div class="form-group">
                                                                <label for="land_mark" class="col-form-label"><?php echo $this->lang->line('ltr_landmark_name');?></label>
                                                                <input class="form-control" type="text" placeholder="<?php echo $this->lang->line('ltr_landmark_name');?>" id="land_mark"name="land_mark" value="<?=(isset($update_landmarks[0]['land_mark']) && !empty($update_landmarks[0]['land_mark']))?$update_landmarks[0]['land_mark']:'';?>">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="form-group">
                                            ...
                                            <table id="example" class="table table-striped table-bordered dt-responsive wrap data_table" style="width:100%">
                                                <thead>
                                                    <tr>
                                                        <th></th>
                                                        <th>#</th>
                                                        <th><?php echo $this->lang->line('ltr_landmark_name');?></th>
                                                ...
                                                <?php
                                                $i=1;
                                                foreach($landmarks_table as $listrow){
                                                ?>
                                                    <tr>
                                                        <td><input type="checkbox"  name="lndmrkIds" value="<?= $listrow['id'];?>"></td>
                                                        <td><?=$i;?></td>
                                                        <td><?=$listrow['land_mark'];?></td>
                                                        <td>
														<label class="switch">
                                                        <input id="landmark_status<?=$listrow['id'];?>" class="switch_button" rowid="<?=$listrow['id'];?>" tables="px_addlandmarks" type="checkbox" <?php if(isset($listrow['status']) && $listrow['status']=="1") { echo "checked"; } ?>>
                                                        ...
                                                        <td class="">
                                                            <div class="relative">
                                                                ...
                                                                <div class="action-option ">
                                                                    <ul>
                                                                        <li>
                                                                            <a href="<?=base_url('admin/landmark')?>?did=<?=$listrow['id'];?>"><i class="far fa-edit mr-2 getid"></i><?php echo $this->lang->line('ltr_edit');?></a>
                                                                        </li>
                                                                        <li>
							...
							<div class="modal-dialog modal-dialog-centered" role="document">
								<div class="modal-content">
									...
									<div class="modal-footer">
										<button type="button" class="btn btn-primary deletes" d_id="<?=$listrow['id'];?>" tables="px_addlandmarks"><?php echo $this->lang->line('ltr_delete_yes_button');?></button>
                        ...
                        ?>
                           ...
                           $('#changeStatus').on('click', function(){
                               ...
                               var lndmrkIds = [];
                                $('input[name="lndmrkIds"]:checked').each(function() {
                                  lndmrkIds.push($(this).val());
                                });
                                if(lndmrkIds.length > 0){
                                    if($('#bulk-stts-chnge').val() =='delete'){
                                      $('#confirmOk').text('Yes, delete')
                                      $('#confirmModalMsg').text('Are you sure you want to deleted Landmarks?')
                                    }
                                    else if($('#bulk-stts-chnge').val() =='inactive'){  $('#confirmModalMsg').text('Are you sure you want to make selected Landmarks Inactive ?')
                                     $('#confirmOk').text('Yes')
                                    }
                                    else{
                                        $('#confirmModalMsg').text('Are you sure you want to make selected Landmarks Active ?')
                                         $('#confirmOk').text('Yes')
                                    }
                                    confirmDialog("Are You sure! You want to remove?", function (){
                                    
                                    $.ajax({
                                        url: "<?= base_url('Admin/changeLandmarkStatus'); ?>",
                                        type: 'POST',
                                        data:{
                                            lndmrkIds : lndmrkIds,
                                     ...
                                     showAlert('No Landmark selected.', 'error', $('#burl').val());
                                }
